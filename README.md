...
- 🎯 Entry points to execute your program using `python -m <alpacas_api>` or `$ alpacas_api` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/f-mendes/alpacas_api/branch/main/graph/badge.svg?token=alpacas_api_token_here)](https://codecov.io/gh/f-mendes/alpacas_api)
[![CI](https://github.com/f-mendes/alpacas_api/actions/workflows/main.yml/badge.svg)](https://github.com/f-mendes/alpacas_api/actions/workflows/main.yml)

Awesome alpacas_api created by f-mendes

## Install

from source
```bash
git clone https://github.com/f-mendes/alpacas_api alpacas_api
cd alpacas_api
make install
```
...
```bash
pip install alpacas_api
```

## Executing

```bash
$ alpacas_api run --port 8080
```

or

```bash
python -m alpacas_api run --port 8080
```

or

```bash
$ uvicorn alpacas_api:app
```
...
```bash
❯ alpacas_api --help
Usage: alpacas_api [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ alpacas_api create-user --help
Usage: alpacas_api create-user [OPTIONS] USERNAME PASSWORD
...
```bash
alpacas_api create-user admin admin --superuser
```
...
Run with `alpacas_api run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `alpacas_api create-user`.
...
-----------------------------------------------------
alpacas_api/__init__.py              4      0   100%
alpacas_api/app.py                  16      1    94%
alpacas_api/cli.py                  21      0   100%
alpacas_api/config.py                5      0   100%
alpacas_api/db.py                   10      0   100%
alpacas_api/models/__init__.py       0      0   100%
alpacas_api/models/content.py       47      1    98%
alpacas_api/routes/__init__.py      11      0   100%
alpacas_api/routes/content.py       52     25    52%
alpacas_api/routes/security.py      15      1    93%
alpacas_api/routes/user.py          52     26    50%
alpacas_api/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from alpacas_api.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in alpacas_api/default.toml.

### As environment variables
```bash
export alpacas_api_KEY=value
export alpacas_api_KEY="@int 42"
export alpacas_api_KEY="@jinja {{ this.db.uri }}"
export alpacas_api_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
alpacas_api_ENV=production alpacas_api run
```
